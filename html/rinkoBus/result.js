...
get = async (stopId) => {
    ...
    const url = `/api/rinkoBus/get_departures?id=${stopId}`;
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    if (localStorage.getItem('favorites')) {
        ...
        for (let i = 0; i < favorites.length; i++) {
            if (favorites[i].operator === 'rinkoBus' && favorites[i].stop_id === stopId) {
...
update = async (trips, stopId) => {
    try {
        ...
        const response = await fetch(`/api/rinkoBus/get_rt_data`, {
            method: 'POST',
            headers: {
...
document.getElementById('add-to-favorite').addEventListener('click', async () => {
    ...
    const url = `/api/rinkoBus/search?query=${encodeURIComponent(stopName)}`;
    ...
    try {
        ...
        if (data.length > 0) {
            ...
            if (favorites) {
                ...
                favorites.push({operator: 'rinkoBus', stop_id: result.stop_id, stop_name: result.stop_name, routes: result.routes, date: date});
                favorites.sort((a, b) => b.date - a.date);
            } else {
                favorites = [{operator: 'rinkoBus', stop_id: result.stop_id, stop_name: result.stop_name, routes: result.routes, date: date}];
            }
...
document.getElementById('remove-from-favorite').addEventListener('click', () => {
    ...
    if (favorites) {
        ...
        favorites = favorites.filter(fav => !(fav.operator === 'rinkoBus' && fav.stop_id === stopId));
        localStorage.setItem('favorites', JSON.stringify(favorites));
    }
